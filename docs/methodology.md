...
截至 2026-08-24，本轮主动的 CoolApk／XDA 外部资料搜集已经结束，仓库进入整理和验证阶段。当前文档与 `data/source-index.yaml` 只保存已经筛选保留的来源；未通过筛选、仅重复既有资料或没有可复用技术内容的候选不进入事实库。

结束收集不冻结仓库：既有记录的纠错、上游固定来源的状态变化和可复现的实机结果仍可继续更新。若以后重新扩充来源，应先明确恢复收集范围，再走下面的核验流程。

## 信息生命周期

```mermaid
flowchart TD
    A["明确要求补充的链接或实机现象"] --> B["Issue：待核验"]
    B --> C["固定版本与采集证据"]
    C --> D["区分事实、声明和假设"]
    D --> E["更新 docs 与 data"]
    E --> F["Commit 留存变更历史"]
