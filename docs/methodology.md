...
本仓库不采纳来源不明的转载、删去上下文的截图或无法定位原帖的网盘说明。CoolApk（酷安）来源默认排除；例外必须在 `data/source-index.yaml` 中逐条列出账号、URL、主题范围和验证状态。例外只允许保留线索及归属，不会因为被收录就提高证据等级。

截至 2026-08-23 有两个限定例外：用户“皓皓的小月”的 OnePlus 13T ARB1/EDL 引导，以及用户提供截图中的相机移植讨论。它们分别只适用于 [`edl-arb.md`](edl-arb.md) 和 [`camera-porting-clue.md`](camera-porting-clue.md)，不扩展到相关账号的其他帖子、截图或转述。
