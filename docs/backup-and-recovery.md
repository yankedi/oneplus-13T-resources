...
- 仅写 `recovery_b`；最终 ACK、100 MiB 完整回读、AVB 和五项 B 槽启动分区/LUN4 GPT 不变性检查通过，Android B 与 Lineage Recovery 启动已确认。
- `misc` 的完整 1 MiB 备份通过 Android root 获取，不属于 EDL 读取能力；镜像仅本地保存。

注意：本次未写 A、firmware、NV、userdata，未擦除、格式化或切槽；未执行 TWRP 恢复，Recovery 全功能和全盘恢复仍待验收。见 [实机报告](edl-arb1-device-validation-2026-08-26.md)。
