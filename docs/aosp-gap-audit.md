...
| 分类 | 数量 | 含义 |
|---|---:|---|
| `IDENTICAL` | 12 | 已存在同等实现，无需移植 |
| `MISSING` | 4 | 当前树缺少对应实现，但仍需先证明运行时需求 |
| `PARTIALLY_PORTED` | 5 | 两边存在共同基础，只能选择性审计 |
...
1. **UDFPS UI-ready / `notify_fppress` 链路（P0）**：现象是 HAL 等待约 502 ms 后返回错误 `15001`。先追踪 SystemUI、framework、OFP 节点和 SELinux，再决定是否手工移植参考补丁。
2. **指纹手势抢占与休眠遮罩清理（P1）**：与主故障拆开验证，避免将生命周期修复和错误屏蔽混在一起。
3. **CarrierSettings / NR 信号格映射（P1）**：设备已能 NR SA 通话和数据，问题更像产品侧阈值映射，而不是射频或 modem 故障。
